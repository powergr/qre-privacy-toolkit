...
export function CleanerView() {
  ...
  async function cleanAll() {
    ...
    for (const file of files) {
      try {
        const res = await invoke<string>("clean_file_metadata", {
          path: file,
          options: opts,
        });
        cleaned.push(res);
        ...
        {files.length > 0 && (
          <div style={{ width: "100%", textAlign: "left", maxWidth: 600 }}>
                ...
                {analyzingPreview ? (
                  ...
                  <div style={{ display: "grid", gap: 15 }}>
                    ...
                    >
                      <div
                        style={{
                          background: previewReport.has_gps
                            ? "rgba(248, 113, 113, 0.1)"
                            : "rgba(255,255,255,0.05)",
                          padding: 8,
                          borderRadius: 6,
                          color: previewReport.has_gps
                            ? "#f87171"
                            : "var(--text-dim)",
                        }}
                      >
                    ...
                    >
                      <div
                        style={{
                          background: previewReport.has_author
                            ? "rgba(250, 204, 21, 0.1)"
                            : "rgba(255,255,255,0.05)",
                          padding: 8,
                          borderRadius: 6,
                          color: previewReport.has_author
                            ? "#facc15"
                            : "var(--text-dim)",
                        }}
                      >
                    ...
                    >
                      <div
                        style={{
                          background: previewReport.creation_date
                            ? "rgba(255,255,255,0.1)"
                            : "rgba(255,255,255,0.05)",
                          padding: 8,
                          borderRadius: 6,
                          color: previewReport.creation_date
                            ? "var(--text-main)"
                            : "var(--text-dim)",
                        }}
                      >
                    ...
                    >
                      ...
                      >
                        <List size={14} /> {showRaw ? "Hide" : "View"} Full
                        Metadata ({previewReport.raw_tags.length} tags)
                      </button>
              ...
              <div style={{ display: "flex", gap: 10 }}>
                ...
                >
                  <input
                    type="checkbox"
                    checked={opts.gps}
                    onChange={() => setOpts({ ...opts, gps: !opts.gps })}
                  />
                  GPS
                </label>
                <label
                ...
                >
                  <input
                    type="checkbox"
                    checked={opts.author}
                    onChange={() => setOpts({ ...opts, author: !opts.author })}
                  />
                  Info
                </label>
                <label
                ...
                >
                  <input
                    type="checkbox"
                    checked={opts.date}
                    onChange={() => setOpts({ ...opts, date: !opts.date })}
                  />
                  Dates
                </label>
              </div>
            ...
            >
              {loading ? (
                <>
                  <Loader2 className="spinner" size={20} /> Scrubbing...
                </>
              ) : (
                <>
                  <Eraser size={20} />
